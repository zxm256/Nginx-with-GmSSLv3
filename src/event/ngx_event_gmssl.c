...
ngx_module_t ngx_gmssl_module = {NGX_MODULE_V1,
...
ngx_int_t ngx_ssl_ciphers(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_str_t *ciphers,
    ...
    // 是指由服务器优先选择更安全的cipher，而不是依靠客户端的优先顺序
...
ngx_int_t ngx_ssl_client_certificate(ngx_conf_t *cf, ngx_ssl_t *ssl,
    ...
    ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0,
                  "failed: do not support client certificate!", cert->data);
    return NGX_ERROR;
}
...
ngx_int_t ngx_ssl_crl(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_str_t *crl) {
    if (crl->len == 0) return NGX_OK;

    ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0,
                  "failed: do not support client certificate!", crl->data);
    return NGX_ERROR;
}
...
ngx_array_t *ngx_ssl_read_password_file(ngx_conf_t *cf, ngx_str_t *file) {
    ...
    do {
        ...
        if (len == NGX_SSL_PASSWORD_BUFFER_SIZE) {
            ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "too long line in \"%s\"",
                               file->data);
            passwords = NULL;
            goto cleanup;
        }
...
ngx_int_t ngx_ssl_ecdh_curve(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_str_t *name) {
    ...
    ngx_ssl_error(NGX_LOG_EMERG, ssl->log, 0, "failed: unknown curve \"%s\"",
                  name->data);
    return NGX_ERROR;
}
...
ngx_int_t ngx_ssl_create_connection(ngx_ssl_t *ssl, ngx_connection_t *c,
    ...
    ngx_ssl_t *ssl_ctx = &sc->ssl_ctx;
    TLS_CTX ctx;
    memset(&ctx, 0, sizeof(ctx));

    if (tls_ctx_init(&ctx, TLS_protocol_tls13, TLS_server_mode) != 1 ||
        tls_ctx_set_cipher_suites(&ctx, server_ciphers,
                                  sizeof(server_ciphers) / sizeof(int)) != 1 ||
        tls_ctx_set_certificate_and_key(&ctx, (char *)ssl_ctx->certfile,
                                        (char *)ssl_ctx->keyfile,
                                        (char *)ssl_ctx->password) != 1) {
        error_print();
        return NGX_ERROR;
    }

    if (tls_init(sc->connection, &ctx) != 1) {
        error_print();
        return NGX_ERROR;
    }

    if (tls_set_socket(sc->connection, c->fd) != 1) {
        ngx_ssl_error(NGX_LOG_ALERT, c->log, 0, "SSL_set_fd() failed");
        return NGX_ERROR;
    }
    if (ngx_nonblocking(c->fd) == -1) {
        ngx_ssl_error(NGX_LOG_ALERT, c->log, 0, "ngx_nonblocking() failed");
        return NGX_ERROR;
    }
    if (flags & NGX_SSL_CLIENT) {
        // 什么情况下nginx作为客户端？可能在某些特定场景下吧，根据NGX_SSL_CLIENT检查一下
        ngx_ssl_error(NGX_LOG_ALERT, c->log, 0,
                      "gmssl in nginx should not run at client");
        return NGX_ERROR;
    }
    ...
    if (SSL_set_ex_data(sc->connection, ngx_ssl_connection_index, c) == 0) {
                                                                                                                                    ngx_ssl_error(NGX_LOG_ALERT,
    c->log, 0, "SSL_set_ex_data() failed"); return NGX_ERROR;
    }
    */
...
ngx_int_t ngx_ssl_set_session(ngx_connection_t *c, ngx_ssl_session_t *session) {
    ...
    if (session) {
        ngx_ssl_error(NGX_LOG_ALERT, c->log, 0,
                      "ngx_ssl_set_session() not supported");
        return NGX_ERROR;
    }
...
ngx_int_t ngx_ssl_handshake(ngx_connection_t *c) {
    ...
    n = tls_do_handshake(c->ssl->connection);
    ...
    if (sslerr == SSL_ERROR_WANT_READ) {
                                                                                                                                    c->read->ready
    = 0; c->read->handler = ngx_ssl_handshake_handler; c->write->handler =
    ngx_ssl_handshake_handler;

                                                                                                                                    if
    (ngx_handle_read_event(c->read, 0) != NGX_OK) { return NGX_ERROR;
                                                                                                                                    }

                                                                                                                                    if
    (ngx_handle_write_event(c->write, 0) != NGX_OK) { return NGX_ERROR;
                                                                                                                                    }
    ...
    err, "peer closed connection in SSL handshake");
    ...
    if (c->ssl->handshake_rejected) {
                                                                                                                                    ngx_connection_error(c,
    err, "handshake rejected"); ERR_clear_error();
...
static void ngx_ssl_handshake_handler(ngx_event_t *ev) {
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, c->log, 0, "SSL handshake handler: %d",
                   ev->write);
...
#define SSL_ERROR_ZERO_RETURN 4
    ...
    (c->ssl->saved_write_handler) {
                                                                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                                                                    c->write->handler
    = c->ssl->saved_write_handler; c->ssl->saved_write_handler = NULL;
                                                                                                                                                                                                                                                                    c->write->ready
    = 1;
    ...
    (ngx_handle_write_event(c->write, 0) != NGX_OK) { return NGX_ERROR;
                                                                                                                                    ...
                                                                                                                                    // we do not
    set the timer because there is already the read event timer

                                                                                                                                    if
    (c->ssl->saved_write_handler == NULL) { c->ssl->saved_write_handler =
    c->write->handler; c->write->handler = ngx_ssl_write_handler;
                                                                                                                                    }
    ...
    if (sslerr == SSL_ERROR_ZERO_RETURN || ERR_peek_error() == 0) {
                                                                                                                                    ngx_log_debug0(NGX_LOG_DEBUG_EVENT,
    c->log, 0, "peer shutdown SSL cleanly"); return NGX_DONE;
    }
    */
...
ngx_chain_t *ngx_ssl_send_chain(ngx_connection_t *c, ngx_chain_t *in,
    ...
    for (;;) {
        while (in && buf->last < buf->end && send < limit) {
            ...
            ngx_log_debug1(NGX_LOG_DEBUG_EVENT, c->log, 0, "SSL buf copy: %z",
                           size);
...
{
                                                                                                                                ngx_connection_t
*c; ngx_connection_handler_pt     handler;
...
c->ssl->handler;
                                                                                                                                ...
                                                                                                                                if
(ev->timedout) { c->timedout = 1;
                                                                                                                                }

                                                                                                                                ngx_log_debug0(NGX_LOG_DEBUG_EVENT,
ev->log, 0, "SSL shutdown handler");

                                                                                                                                if
(ngx_ssl_shutdown(c) == NGX_AGAIN) { return;
                                                                                                                                }
...
ngx_int_t ngx_ssl_session_cache_init(ngx_shm_zone_t *shm_zone, void *data) {
    ...
    ngx_rbtree_init(&cache->session_rbtree, &cache->sentinel, NULL);
...
void ngx_ssl_cleanup_ctx(void *data) { error_print(); }
