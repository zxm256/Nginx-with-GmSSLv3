...
#define ngx_ssl_session_t       int // 这个可能需要一个 我们现在可能不支持SESSION 了
#define ngx_ssl_conn_t          TLS_CONNECT   // 这个应该是 TLS_CONNECT
...
struct ngx_ssl_connection_s {
    ngx_ssl_t                   ssl_ctx;
    ngx_ssl_conn_t             *connection; // TLS_CONNECT 实际上没有必要malloc
...
ngx_int_t ngx_ssl_ocsp(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_str_t *responder, ngx_uint_t depth, ngx_shm_zone_t *shm_zone);
ngx_int_t ngx_ssl_ocsp_resolver(ngx_conf_t *cf, ngx_ssl_t *ssl, ngx_resolver_t *resolver, ngx_msec_t resolver_timeout);
...
ngx_int_t ngx_ssl_session_cache(ngx_ssl_t *ssl, ngx_str_t *sess_ctx, ngx_array_t *certificates, ssize_t builtin_session_cache, ngx_shm_zone_t *shm_zone, time_t timeout);
...
void ngx_ssl_free_session(ngx_ssl_session_t *sess);
ngx_connection_t *ngx_ssl_get_connection(ngx_ssl_conn_t *c);
...
*/
ngx_int_t ngx_ssl_verify_error_optional(long n);
